    		 ...
    		 //	 printf("Get %dth UDP packet\n", ++(*id));
     		 //	 printf("packet length: %d\n", pkthdr->len);
	   		 ...
	   		 //	 printf("Received time: %s, %6ld\n", ctime((const time_t*)&pkthdr->ts.tv_sec), pkthdr->ts.tv_usec);
				 ...
				 FILE *fpt;
				 fpt = fopen("timestamp0506.txt", "a");
				 fprintf(fpt, "%s %ld\n", inet_ntoa(ip->ip_src), (pkthdr->ts.tv_sec*1000000 + pkthdr->ts.tv_usec));
				 fclose(fpt);
		 }
	 }
}
...
{
	int id = 0;
	pcap_loop(device, 2000, getPacket, (u_char*)&id);
}
...
int main()
{
  ...
  if(pcap_compile(device, &fp, "", 1, 0) == -1)
	{
